...
class MainActivity extends React.Component {
  ...
  renderActividades = () => {
    return this.props.row.hijitas.map((h, i) => (
      <Activity content={h} key={i} showText={this.showText} />
    ));
  };
  handleSwitch = () => {
  ...
  render() {
    ...
    return (
      <Card style={{ marginBottom: "20px" }}>
        <CardContent>
          <span>
            <strong>{row["usuario"]}</strong> {row["fecha"]}
          </span>
          {/*
          ...
          <p>
            <strong>Concepto:</strong> {row["concepto"]}{" "}
          </p>
          {row["idLote"] && (
            <p>
              <strong>Id del lote:</strong>{" "}
              {this.showText(row["idLote"], "Obs")}
            </p>
          )}

          <p>
            <strong>Observación:</strong>{" "}
            {this.showText(row["observacion"], "Obs")}
          </p>

          <div style={{ position: "relative" }}>
            {" "}
            <strong style={{ display: "inline" }}>Archivo: </strong>
            {row["URLArchivo"] === "" || !row["URLArchivo"] ? (
              <span>No disponible</span>
            ) : (
              <a className="downloadAvailable" href={row["URLArchivo"]}>
                Descargar
              </a>
            )}
            {row["cambiosCasos"] && row["cambiosCasos"].length > 0 && (
              <div className="downloadAvailable" onClick={this.openResultModal}>
                Mostrar resultado
              </div>
            )}
            {row["concepto"] !== "Cambio manual a estado de casos" && (
              <span className="text-right">
                <span className="downloadAvailable" onClick={this.handleSwitch}>
                  {this.state.showActivities ? "Ocultar" : "Ver"} respuestas
                </span>
              </span>
            )}
          </div>
          {this.state.showActivities && (
            <div className="activities-wrapper">
              ...
              >
                <CardContent>Nueva respuesta</CardContent>
              </Card>
              <NewActivityModal
