...
class LookOneCaseModal extends React.Component {
    ...
    submit = () => {
        ...
        if (this.state.id.trim() != "") {
            fetch("https://intellgentcms.herokuapp.com/sica/api/caso/" + this.state.id.trim(), {
                method: "GET",
                headers: {
                ...
                json => {
                    if (json.success) {
                        if (json.caso) {
                            this.setState({ loading: false, error: "", currentCase: json.caso })
                        } else {
                            this.setState({ loading: false, error: "No se encontró un caso con ese número de orden." })
                        }
                    } else {
