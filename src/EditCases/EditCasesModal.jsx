...
class EditCasesModal extends React.Component {
  ...
  componentDidMount() {
    fetch(`http://localhost:3001/sica/api/diccionarioDeEstados`, {
      method: "GET",
      headers: {
      ...
      .then((json) => this.setState({ statesByModule: json.estadosPorModulo }));
  }
  ...
  handleTextChange = (e) => {
    ...
    if (text.includes("\n")) {
      ...
      for (let i = 0; i < newCases.length; i++) {
        if (
          !Number.parseInt(newCases[i]) ||
          Number.parseInt(newCases[i]).toString() !== newCases[i]
        ) {
      ...
      if (!this.checkForDuplicates(newCases)) {
        this.setState((prevState) => {
          return { cases: [...newCases, ...prevState.cases] };
        });
      }
  ...
  handleKeyType = (e) => {
    ...
    if (e.key === "Enter") {
      ...
      if (!this.checkForDuplicates([text])) {
        this.setState((prevState) => {
          return { cases: [text, ...prevState.cases] };
        });
      }
      this.setState({ idValues: "" });
    }
          ...
          .then((json) => {
            if (json.success) {
              this.setState({
                ...
                casosRestantes: json.casosRestantes,
              });
            } else {
              this.setState({
                loading: false,
      ...
      >
        <div className={classes.modal}>
          ...
          <FormControl className="edit-case-modal-field">
            <InputLabel htmlFor="module-label-placeholder">Módulo</InputLabel>
            <Select
